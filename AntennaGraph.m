classdef AntennaGraph
    ...
    methods
        ...
        function M = getSegmentCount(obj)
            M = size(obj.Edges, 1);
        end
        
        
        function L = getSegmentLength(obj, edgeIdx)
            E = obj.Edges(edgeIdx, :);
            V = obj.Vertices;
            p1 = V(E(:,1), :);
            p2 = V(E(:,2), :);
        ...
        function p_mid = getSegmentMidpoint(obj, edgeIdx)
            E = obj.Edges(edgeIdx, :);
            V = obj.Vertices;
            p1 = V(E(:,1), :);
            p2 = V(E(:,2), :);
        ...
        function t = getSegmentTangent(obj, edgeIdx)
            E = obj.Edges(edgeIdx, :);
            V = obj.Vertices;
            p1 = V(E(:,1), :);
            p2 = V(E(:,2), :);
        ...
        function props = getSegmentProperties(obj, edgeIdx)
            E = obj.Edges(edgeIdx, :);
            V = obj.Vertices;
            v1_idx = E(:,1);
            v2_idx = E(:,2);
            v1 = V(v1_idx, :);
            v2 = V(v2_idx, :);
            L = obj.getSegmentLength(edgeIdx);
            mid = obj.getSegmentMidpoint(edgeIdx);
            t = obj.getSegmentTangent(edgeIdx);
            lenCell = num2cell(L(:));
            midCell = num2cell(mid, 2);
            tanCell = num2cell(t, 2);
            v1IdxCell = num2cell(v1_idx);
            v2IdxCell = num2cell(v2_idx);
            v1Cell = num2cell(v1, 2);
            v2Cell = num2cell(v2, 2);
            props = struct('length', lenCell, ...
                'midpoint', midCell, ...
                'tangent', tanCell, ...
                'v1_idx', v1IdxCell, ...
                'v2_idx', v2IdxCell, ...
        ...
        function props = getAllSegmentProperties(obj)
            M = size(obj.Edges, 1);
            if M == 0
                props = struct('length',{},'midpoint',{},'tangent',{},'v1_idx',{},'v2_idx',{},'v1',{},'v2',{});
                return;
            end
            idx = (1:M).';
            props = obj.getSegmentProperties(idx);
        end
